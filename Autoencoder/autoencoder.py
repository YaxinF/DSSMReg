...
input_file = r'ExpressionData.csv'
...
for run in range(10):
    ...
    embedding_df.to_csv('hESC/gene_exp' + str(run) + '.csv')

    # Record models
    model_json = std_model._encoder.to_json()
    with open('hESC/AE_encoder_' + str(latent_dim) + 'L_fold' + str(run) + '.json', "w") as json_file:
        json_file.write(model_json)
    std_model._encoder.save_weights('hESC/AE_encoder_' + str(latent_dim) + 'L_fold' + str(run) + '.h5')
    print("Saved model to disk")
    model_json = std_model._decoder.to_json()
    with open('hESC/AE_decoder_' + str(latent_dim) + 'L_fold' + str(run) + '.json', "w") as json_file:
        json_file.write(model_json)
    std_model._decoder.save_weights('hESC/AE_decoder_' + str(latent_dim) + 'L_fold' + str(run) + '.h5')
    print("Saved model to disk")